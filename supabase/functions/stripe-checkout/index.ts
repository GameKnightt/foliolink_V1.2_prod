...
// Validation critique des clés API
if (!stripeSecret) {
...
Deno.serve(async (req) => {
  try {
    ...
    /**
     * Check if customer exists in Stripe, create new one if not found or invalid
     */
    let shouldCreateNewCustomer = !customer || !customer.customer_id;
    ...
    if (customer && customer.customer_id) {
      try {
        ...
        console.warn(`Stripe customer ${customer.customer_id} not found or invalid, creating new one:`, stripeError.message);
        shouldCreateNewCustomer = true;
        
        // Mark old customer as deleted in our database
        await supabase
          .from('stripe_customers')
