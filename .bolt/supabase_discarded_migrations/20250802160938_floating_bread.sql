/*
  ...
  1. New Tables
    - `apprentissage_categories`
      ...
      - `competence_id` (text, competence identifier)
      - `level` (integer, competence level)
...
CREATE TABLE IF NOT EXISTS apprentissage_categories (
  ...
  user_id uuid REFERENCES auth.users(id) ON DELETE SET NULL, -- Corrected reference to auth.users
