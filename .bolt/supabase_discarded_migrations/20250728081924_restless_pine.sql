/*
  # Create example data tables for portfolio demonstration

  1. New Tables
    - `example_competences` - Example competencies for demo
    - `example_apprentissages` - Example learning outcomes  
    - `example_projets` - Example projects
    - `example_preuves` - Example evidence/proofs
    - `example_fichiers_projets` - Example project files

  2. Security
    - Enable RLS on all example tables
    - Add public read policies for demo data

  3. Sample Data
    - Insert example competencies for BUT GEII
    - Insert example projects and learning outcomes
    - Insert example evidence and proofs
*/
...
DROP POLICY IF EXISTS "Anyone can read example competences" ON example_competences;
DROP POLICY IF EXISTS "Anyone can read example apprentissages" ON example_apprentissages;
DROP POLICY IF EXISTS "Anyone can read example projets" ON example_projets;
DROP POLICY IF EXISTS "Anyone can read example preuves" ON example_preuves;
DROP POLICY IF EXISTS "Anyone can read example fichiers projets" ON example_fichiers_projets;
...
CREATE POLICY "Anyone can read example fichiers projets"
  ON example_fichiers_projets
  FOR SELECT
  TO public
  USING (true);

-- Insérer les données d'exemple seulement si elles n'existent pas déjà
INSERT INTO example_competences (id, title, subtitle, description, levels, color, icon) 
SELECT * FROM (VALUES
...
) AS v(id, title, subtitle, description, levels, color, icon)
WHERE NOT EXISTS (SELECT 1 FROM example_competences WHERE example_competences.id = v.id);

-- Insérer les projets d'exemple
INSERT INTO example_projets (titre, description, icone, niveau, duree, fonctionnalites, technologies, statut) 
SELECT * FROM (VALUES
...
) AS v(titre, description, icone, niveau, duree, fonctionnalites, technologies, statut)
WHERE NOT EXISTS (SELECT 1 FROM example_projets WHERE example_projets.titre = v.titre);

-- Insérer les apprentissages d'exemple
INSERT INTO example_apprentissages (competence_id, level, title, description, evaluation, argumentaire) 
SELECT * FROM (VALUES
...
) AS v(competence_id, level, title, description, evaluation, argumentaire)
WHERE NOT EXISTS (SELECT 1 FROM example_apprentissages WHERE example_apprentissages.title = v.title);
...
WHERE NOT EXISTS (
  SELECT 1 FROM example_preuves ep 
  WHERE ep.apprentissage_id = ea.id
);